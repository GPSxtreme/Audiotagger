 ...
 * audio frames. Because we are not trying to play the audio but only extract
 * some information
 * regarding the audio we only need to read the first audio frames to ensure
 * that we have correctly
 * identified them as audio frames and extracted the metadata we reuire.
 *
 * <p>
 * Start of Audio id 0xFF (11111111) and then second byte anded with
 * 0xE0(11100000).
 * For example 2nd byte doesnt have to be 0xE0 is just has to have the top 3
 * signicant
 * bits set. For example 0xFB (11111011) is a common occurence of the second
 * match. The 2nd byte
 * defines flags to indicate various mp3 values.
 *
 * <p>
 * Having found these two values we then read the header which comprises these
 * two bytes plus a further
 * two to ensure this really is a MP3Header, sometimes the first frame is
 * actually a dummy frame with summary information
 * held within about the whole file, typically using a Xing Header or LAme
 * Header. This is most useful when the file
 * is variable bit rate, if the file is variable bit rate but does not use a
 * summary header it will not be correctly
 * identified as a VBR frame and the track length will be incorrectly
 * calculated. Strictly speaking MP3 means
 * Layer III file but MP2 Layer II), MP1 Layer I) and MPEG-2 files are sometimes
 * used and named with
...
public class MP3AudioHeader implements AudioHeader {
    ...
    /**
     * After testing the average location of the first MP3Header bit was at 5000
     * bytes so this is
     * why chosen as a default.
     */
    private final static int FILE_BUFFER_SIZE = 5000;
    private final static int MIN_BUFFER_REMAINING_REQUIRED = MPEGFrameHeader.HEADER_SIZE
            + XingFrame.MAX_BUFFER_SIZE_NEEDED_TO_READ_XING;
     ...
     *
     * The search starts from the start of the file, it is usually safer to use the
     * alternative constructor that
     * allows you to provide the length of the tag header as a parameter so the tag
     * can be skipped over.
     *
     * @param seekFile
     * @throws IOException
     ...
     * Search for the first MP3Header in the file
     *
     * Starts searching from location startByte, this is because there is likely to
     * be an ID3TagHeader
     * before the start of the audio. If this tagHeader contains unsynchronized
     * information there is a
     * possibility that it might be inaccurately identified as the start of the
     * Audio data. Various checks
     * are done in this code to prevent this happening but it cannot be guaranteed.
     *
     * Of course if the startByte provided overstates the length of the tag header,
     * this could mean the
     * start of the MP3AudioHeader is missed, further checks are done within the MP3
     * class to recognize
    ...
    public MP3AudioHeader(final File seekFile, long startByte) throws IOException, InvalidAudioFrameException {
        if (!seek(seekFile, startByte)) {
            // throw new
            InvalidAudioFrameException(ErrorMessage.NO_AUDIO_HEADER_FOUND.getMsg(seekFile.getName()));
        }
    }
     ...
     * @param startByte if there is an ID3v2tag we dont want to start reading from
    ...
    public boolean seek(final File seekFile, long startByte) throws IOException {
        ...
        try {
            do {
                // TODO remaining() is quite an expensive operation, isn't there a way we can
                // work this out without
                // interrogating the bytebuffer. Also this is rarely going to be true, and could
                // be made less true
                ...
                if (MPEGFrameHeader.isMPEGFrame(bb)) {
                    try {
                        ...
                        syncFound = true;
                        // if(2==1) use this line when you want to test getting the next frame without
                        // using xing
                        ...
                        // There is a small but real chance that an unsynchronised ID3 Frame could fool
                        // the MPEG
                        // Parser into thinking it was an MPEG Header. If this happens the chances of
                        // the next bytes
                        // forming a Xing frame header are very remote. On the basis that most files
                        // these days have
                        // Xing headers we do an additional check for when an apparent frame header has
                        // been found
                        // but is not followed by a Xing Header:We check the next header this wont
                        // impose a large
                        // overhead because wont apply to most Mpegs anyway ( Most likely to occur if
                        // audio
                ...
                // TODO position() is quite an expensive operation, isn't there a way we can
                // work this out without
         ...
         * {
         * logger.severe(seekFile.getName()+"length:"+startByte+"Difference:"+(
         * filePointerCount - startByte));
         * }
         */
        return syncFound;
    ...
    /**
     * Called in some circumstances to check the next frame to ensure we have the
     * correct audio header
     *
     * @param seekFile
    ...
    private boolean isNextFrameValid(File seekFile, long filePointerCount, ByteBuffer bb, FileChannel fc)
            throws IOException {
        if (MP3AudioHeader.logger.isLoggable(Level.FINEST)) {
            MP3AudioHeader.logger.finer("Checking next frame" + seekFile.getName() + ":fpc:" + filePointerCount
                    + "skipping to:" + (filePointerCount + mp3FrameHeader.getFrameLength()));
        }
        ...
        if (bb.remaining() <= MIN_BUFFER_REMAINING_REQUIRED + mp3FrameHeader.getFrameLength()) {
            ...
            fc.read(bb, fc.position());
            bb.flip();
            // So now original buffer has been replaced, so set current position to start of
            // buffer
            currentPosition = 0;
            // Not enough left
            if (bb.limit() <= MIN_BUFFER_REMAINING_REQUIRED) {
    ...
    /**
     * Set number of frames in this file, use Xing if exists otherwise ((File Size -
     * Non Audio Part)/Frame Size)
     */
    ...
    /**
     * @return The number of frames within the Audio File, calculated as accurately
     *         as possible
     */
    ...
    /**
     * @return The number of frames within the Audio File, calculated by dividing
     *         the filesize by
     *         the number of frames, this may not be the most accurate method
     *         available.
     */
    ...
    /**
     * Set the time each frame contributes to the audio in fractions of seconds, the
     * higher
    ...
    protected void setTimePerFrame() {
        ...
        // Because when calculating framelength we may have altered the calculation
        // slightly for MPEGVersion2
        // to account for mono/stereo we seem to have to make a corresponding
        // modification to get the correct time
    ...
    /**
     * @return the the time each frame contributes to the audio in fractions of
     *         seconds
     */
     ...
     * Calculation is Number of frames multiplied by the Time Per Frame using the
     * first frame as a prototype
     * Time Per Frame is the number of samples in the frame (which is defined by the
     * MPEGVersion/Layer combination)
     * divided by the sampling rate, i.e the higher the sampling rate the shorter
     * the audio represented by the frame is going
     * to be.
     */
    ...
    public String getTrackLengthAsString() {
        ...
        try {
            ...
            logger.warning(
                    "Unable to parse:" + getPreciseTrackLength() + " failed with ParseException:" + pe.getMessage());
            return "";
        }
    }
    ...
    /**
     * @return bitrate in kbps, no indicator is provided as to whether or not it is
     *         vbr
     */
    ...
    /**
     * @return if the bitrate is variable, Xing header takes precedence if we have
     *         one
     */
